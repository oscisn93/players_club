...
export default function ScoresPage() {
  ...
  useEffect(() => {
    if (selectedGame === "All") {
      ...
      setFilteredScores(mockScores.filter((score) => score.game === selectedGame))
    }
  }, [selectedGame])

  return (
    <div className="container mx-auto px-4 py-12">
      <h1 className="text-4xl font-bold mb-8 text-emerald-400">Leaderboard</h1>
      <div className="mb-6">
      ...
      <Table className="bg-zinc-800 rounded-lg overflow-hidden">
        ...
        <TableHeader>
          <TableRow>
            ...
            <TableHead className="text-emerald-300 text-right">Score</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {filteredScores.map((score, index) => (
            <TableRow key={score.id}>
              <TableCell className="font-medium text-emerald-200">{index + 1}</TableCell>
              <TableCell className="text-emerald-200">{score.username}</TableCell>
              <TableCell className="text-emerald-200">{score.game}</TableCell>
              <TableCell className="text-right text-emerald-200">{score.score}</TableCell>
            </TableRow>
          ))}
        </TableBody>
      </Table>
